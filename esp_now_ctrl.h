...
#define ESP_NOW_PENDING_QUEUE_SIZE 4
struct_message espNowPendingQueue[ESP_NOW_PENDING_QUEUE_SIZE];
volatile uint8_t espNowPendingHead = 0;
volatile uint8_t espNowPendingTail = 0;
volatile uint8_t espNowPendingCount = 0;
volatile uint32_t espNowPendingDropped = 0;
portMUX_TYPE espNowPendingMux = portMUX_INITIALIZER_UNLOCKED;
...
void espNowQueueMessage(const unsigned char* incomingData, int len) {
  struct_message incoming;
  memset(&incoming, 0, sizeof(incoming));
  memcpy(&incoming, incomingData, min(len, (int)sizeof(incoming)));

  portENTER_CRITICAL(&espNowPendingMux);
  if (espNowPendingCount >= ESP_NOW_PENDING_QUEUE_SIZE) {
    espNowPendingTail = (espNowPendingTail + 1) % ESP_NOW_PENDING_QUEUE_SIZE;
    espNowPendingCount--;
    espNowPendingDropped++;
  }
  espNowPendingQueue[espNowPendingHead] = incoming;
  espNowPendingHead = (espNowPendingHead + 1) % ESP_NOW_PENDING_QUEUE_SIZE;
  espNowPendingCount++;
  portEXIT_CRITICAL(&espNowPendingMux);
}

bool espNowDequeueMessage(struct_message* output) {
  bool hasMessage = false;
  portENTER_CRITICAL(&espNowPendingMux);
  if (espNowPendingCount > 0) {
    *output = espNowPendingQueue[espNowPendingTail];
    espNowPendingTail = (espNowPendingTail + 1) % ESP_NOW_PENDING_QUEUE_SIZE;
    espNowPendingCount--;
    hasMessage = true;
  }
...
void espNowHandlePendingCommand() {
  struct_message pendingMessage;
  if (!espNowDequeueMessage(&pendingMessage)) {
    return;
  }

  espNowMegsRecv = pendingMessage;
...
void OnDataRecv(const unsigned char* mac, const unsigned char* incomingData, int len) {
  ...
  espNowQueueMessage(incomingData, len);
}
